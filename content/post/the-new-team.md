...
While I know that the goal of any team is not to keep everybody fully utilized, when we think in terms of applying Lean to our process we really try to find the wait queues (idle time) and eliminate them.
...
Due to the need to create a lot of requirements for the developers and also for the need to review and make slight modifications to what AI produces, they will be very busy and will not have a lot of time to have meetings with outside stakeholders. As a result, the organization eneds to empower these POs to make decisions without having extensive consultation outside of the team.
...
Developers will not be writing code. Instead they will be using an AI Agent like Claude Code to product the code and tests. In order to make this successful, the developers will need to truly understand the agents they are using, understand how to place quality/testing constraints and specific workflow instructions.
