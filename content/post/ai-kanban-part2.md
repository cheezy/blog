...
In Part 2 of this series I will be using Claude Code to generate a plan to build the Kanban Board application. It is very important to me that the plan take very small steps and include a lot of places where we check the quality along the way. Here is a video of the session. Following the video I include the prompts I used as well as the entire plan created by Claude.

{{< youtube _07ZxEppbAE >}}

### The Prompts

I actually added three prompts during this session. The first laid out the high level details of the application I want to build. The prompt is:

_I want to build a Kanban Board Application. Users should be able to create a new kanban board. User should be able to add columns to the board and provide a name for the columns. Users shuld be able to add tasks to the columns. users should be able to move tasks from one column to another. Users should have to authenticate in order to access their boards. Pleae create a plan and write it to a file named PLAN.md so we can keep track of our progress._

Ones this prompt completed I asked Claude to add more details. Here's the prompt:

_I would like the plan to be more detailed. Please add a step after phase 1 to create the schemas. Also ensure you include the quality checks outlined in the AGENTS.md file._

Finally I asked Claude to turn it into a checklist style plan. Here's the prompt:

_Please update the plan so that we can check off the tasks that we complete._


### The Plan

This is the entire plan that was generated by Claude Code. It is very long but I am including it so you can see how much details were added.
